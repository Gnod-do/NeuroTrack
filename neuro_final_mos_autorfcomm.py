...
# ==============================
def find_bt_mac_by_name(name_keywords=("NeuroTrack", "Trackduino")) -> Optional[str]:
    """
    Tìm MAC theo tên thiết bị từ `bluetoothctl devices`.
    Trả về MAC (AA:BB:CC:DD:EE:FF) hoặc None nếu không thấy.

    Ghi chú:
    - Thiết bị cần được pair (và thường là trust) trước thì mới hiện trong danh sách.
    """
    try:
        p = subprocess.run(
    ...
    for line in p.stdout.splitlines():
        ...
        if any(k.lower() in name.lower() for k in name_keywords):
            return mac
    return None


def ensure_rfcomm_by_name(
    idx: int = 0,
    name_keywords=("NeuroTrack", "Trackduino"),
    channel: Optional[int] = None,
    use_sudo: bool = True,
) -> bool:
    """
    Nếu chưa có /dev/rfcomm{idx} thì:
    - dò MAC theo tên thiết bị trong bluetoothctl
    - rfcomm release idx (để tránh trạng thái kẹt)
    - rfcomm bind idx MAC [channel]

    Trả True nếu /dev/rfcomm{idx} tồn tại sau đó.
    """
    dev = f"/dev/rfcomm{idx}"
    if os.path.exists(dev):
        return True

    mac = find_bt_mac_by_name(name_keywords)
    if not mac:
        return False

    release_cmd = ["rfcomm", "release", str(idx)]
    bind_cmd = ["rfcomm", "bind", str(idx), mac]
    if channel is not None:
        bind_cmd.append(str(channel))

    if use_sudo:
        release_cmd.insert(0, "sudo")
        bind_cmd.insert(0, "sudo")

    # ignore release errors
    try:
        subprocess.run(release_cmd, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, timeout=3)
    except Exception:
        pass

    try:
        subprocess.run(bind_cmd, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, timeout=6)
    except Exception:
        return False
...
def _bt_name_keywords_from_env(default=("NeuroTrack", "Trackduino")):
    """
    Cho phép cấu hình tên thiết bị qua biến môi trường:
      - NT_BT_NAME="Trackduino_05923"  (1 keyword)
      - NT_BT_NAME="NeuroTrack,Trackduino" (nhiều keyword, phân tách bằng dấu phẩy)
    """
    raw = os.getenv("NT_BT_NAME", "").strip()
    if not raw:
...
class BridgeUI:
    def __init__(self, root):
        ...
        # --- Auto bind rfcomm0 (Bluetooth SPP) theo tên thiết bị, để app tự thấy /dev/rfcomm0 ---
        # Chỉ cần thiết bị đã pair/trust trước đó.
        if platform.system().lower() != "windows":
            keywords = _bt_name_keywords_from_env()
            # thử vài lần vì lúc mới mở app, bluetoothctl có thể chưa liệt kê kịp
            for _ in range(3):
                if ensure_rfcomm_by_name(idx=0, name_keywords=keywords, channel=None, use_sudo=True):
                    break
                time.sleep(0.4)
    ...
    def refresh_ports(self):
        ...
        # AUTO_SELECT_RFCOMM0:
        # Nếu user chưa chọn Trackduino lần nào, ưu tiên chọn /dev/rfcomm0 nếu có
        if track_index < 0 and not prev_track:
            for i, item in enumerate(track_items):
                if item.startswith("/dev/rfcomm0 " ) or item.startswith("/dev/rfcomm0—") or item.startswith("/dev/rfcomm0 —"):
                    track_index = i
                    break
    ...
    def on_ports_timer(self):
        # Nếu rfcomm0 bị mất (Bluetooth reconnect), thử bind lại trước khi refresh
        if platform.system().lower() != "windows":
            keywords = _bt_name_keywords_from_env()
            ensure_rfcomm_by_name(idx=0, name_keywords=keywords, channel=None, use_sudo=True)
    ...
    def connect_track(self):
        ...
        # Ưu tiên /dev/rfcomm0 (Bluetooth SPP) nếu đang tồn tại và user chưa chọn gì
        if (not track) and os.path.exists("/dev/rfcomm0"):
            track = "/dev/rfcomm0"
